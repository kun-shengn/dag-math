...
# ---------------------------------------------------------
# Qwen2.5-math-7B的大模型服务实现
# ---------------------------------------------------------

class LLM_response_Qwen2_5_math_7B(LLM_response):
    """
    基于 OpenAI SDK 的 Qwen2.5-math-7B 回复实现。
    """
    def __init__(self):
        self.llmconfig = LLMConfig_Qwen2_5_math_7B()
        self.client = OpenAI(base_url = self.llmconfig.base_url, api_key = self.llmconfig.api_key)
        self.model_name = self.llmconfig.model_name
    ...
    def generate_response(self, content: str = 'Find the sum of all integer bases $b>9$ for which $17_{b}$ is a divisor of $97_{b}$.', temperature: float = 0) -> str:
        try:
            response = self.client.chat.completions.create(
                model = self.model_name,
                messages = [
                    {"role": "system", "content": qwen2_5_math_7B_prompt_system},
                    {"role": "user", "content": content}
                ],
                temperature = temperature
            )
...
# ---------------------------------------------------------
# qwen3.5-plus的大模型服务实现(API实现)
# ---------------------------------------------------------

class LLM_response_Qwen3_5_Plus(LLM_response):
    """
    基于 OpenAI SDK 的 Qwen3.5-Plus 回复实现。
    """
    def __init__(self):
        self.llmconfig = LLMConfig_Qwen3_5_Plus()
        self.client = OpenAI(base_url = self.llmconfig.base_url, api_key = self.llmconfig.api_key)
        self.model_name = self.llmconfig.model_name
    ...
    def generate_response(self, content: str = 'Find the sum of all integer bases $b>9$ for which $17_{b}$ is a divisor of $97_{b}$.', temperature: float = 0) -> str:
        try:
            response = self.client.chat.completions.create(
                model = self.model_name,
                messages = [
                    {"role": "system", "content": qwen3_5_plus_prompt_system},
                    {"role": "user", "content": content}
                ],
                temperature = temperature
            )
    ...
    def generate_response_nolog(self, content: str = 'Find the sum of all integer bases $b>9$ for which $17_{b}$ is a divisor of $97_{b}$.', temperature: float = 0) -> str:
        try:
            response = self.client.chat.completions.create(
                model = self.model_name,
                messages = [
...
if __name__ == "__main__":
    ...
    # # QWEN2.5-math-7B 模型测试
    # Qwen2_5_math_7B_response = LLM_response_Qwen2_5_math_7B()
    # logger.info(f"Qwen2.5-math-7B 模型: {Qwen2_5_math_7B_response.generate_response_nothinking()}")

    # QWEN3.5-plus 模型测试
    Qwen3_5_plus_response = LLM_response_Qwen3_5_Plus()
    logger.info(f"Qwen3.5-plus 模型: {Qwen3_5_plus_response.generate_response_nothinking()}")